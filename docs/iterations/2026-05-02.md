...
### Iteration 11 Detail — 2026-05-02

**Theme:** Cash-Aware Constraints (Slice 6)

**What We Did:**
Added structured proposal warnings, applied global minimum trade-size suppression, rejected negative cash during proposal generation, documented fixtures, and updated README status.

**Decisions & Rationales:**
Minimum trade constraints should not abort otherwise useful proposals; warnings provide the bridge to later simulation, explanation, and audit slices.

**Files Touched:**
`src/models/domain.ts`, `src/core/trades.ts`, `tests/trades.test.ts`, `tests/fixtures/README.md`, `README.md`, `BUILD_JOURNEY.md`

**Open Questions:**
Should future policies support per-instrument minimum trade sizes?

**Next Steps:**
Proceed to Slice 7: Post-Trade Simulation.


### Iteration 12 Detail — 2026-05-02

**Theme:** Post-Trade Simulation (Slice 7)

**What We Did:**
Added exact trade replay simulation with post-trade holdings, valuation, weights, residual drift, sell-side turnover, oversell checks, and cash reconciliation checks.

**Decisions & Rationales:**
Simulation exposes residual drift from suppressed trades, which keeps Slice 6 constraint decisions visible instead of hiding them in proposal generation.

**Files Touched:**
`src/core/simulation.ts`, `src/core/index.ts`, `tests/simulation.test.ts`, `tests/smoke.test.ts`, `README.md`, `BUILD_JOURNEY.md`

**Open Questions:**
Should future output include gross trade value separately from turnover?

**Next Steps:**
Proceed to Slice 8: Explanation Output.


### Iteration 13 Detail — 2026-05-02

**Theme:** Explanation Output (Slice 8)

**What We Did:**
Added deterministic explanation generation from trigger, proposal, warning, and simulation outputs, with tests for no-op, rebalance, and suppressed-trade residual drift cases.

**Decisions & Rationales:**
Explanation output should be assembled from already-computed facts to avoid contradictory financial rationale.

**Files Touched:**
`src/explanation/explanation.ts`, `src/explanation/index.ts`, `tests/explanation.test.ts`, `tests/smoke.test.ts`, `README.md`, `BUILD_JOURNEY.md`

**Open Questions:**
Should explanations later support localization or audience-specific wording?

**Next Steps:**
Proceed to Slice 9: Audit and Reproducibility Record.


### Iteration 14 Detail — 2026-05-02

**Theme:** Audit and Reproducibility Record (Slice 9)

**What We Did:**
Added audit record generation and stable JSON serialization capturing inputs, drift, trigger, proposal, simulation, and explanation outputs. Added replay tests.

**Decisions & Rationales:**
Audit records should receive metadata from orchestration to preserve deterministic pure helpers and fixture replay.

**Files Touched:**
`src/audit/audit.ts`, `src/audit/index.ts`, `src/models/domain.ts`, `tests/audit.test.ts`, `tests/smoke.test.ts`, `README.md`, `BUILD_JOURNEY.md`

**Open Questions:**
Should event IDs later be content-addressed hashes?

**Next Steps:**
Proceed to Slice 10: Batch Scenario Runner / Test Harness.


### Iteration 15 Detail — 2026-05-02

**Theme:** Batch Scenario Runner (Slice 10)

**What We Did:**
Added an offline fixture runner and `npm run scenario:run` command that evaluates all scenarios into success/error JSON results with audit records for successful scenarios.

**Decisions & Rationales:**
Batch output makes existing invalid fixtures useful as deterministic error-path checks instead of special cases that must be excluded.

**Files Touched:**
`src/runner/scenario-runner.ts`, `src/runner/index.ts`, `tests/scenario-runner.test.ts`, `tests/smoke.test.ts`, `package.json`, `README.md`, `BUILD_JOURNEY.md`

**Open Questions:**
Should the runner later support expected-status manifests and output files?

**Next Steps:**
Proceed to Slice 11: Second Strategy Proof Point.


### Iteration 16 Detail — 2026-05-02

**Theme:** Second Strategy Proof Point (Slice 11)

**What We Did:**
Added manual forced-rebalance strategy isolated to trigger logic and tests proving shared valuation, proposal, simulation, and explanation functions work unchanged.

**Decisions & Rationales:**
Strategy extensibility can be proven without adding calendar/date policy decisions before the MVP requires them.

**Files Touched:**
`src/strategy/manual.ts`, `src/strategy/index.ts`, `tests/manual-strategy.test.ts`, `tests/smoke.test.ts`, `README.md`, `BUILD_JOURNEY.md`

**Open Questions:**
Should calendar scheduling be post-MVP or part of a later MVP extension?

**Next Steps:**
Proceed to Slice 12: MVP Hardening and Final Audit.


### Iteration 17 Detail — 2026-05-02

**Theme:** MVP Hardening and Final Audit (Slice 12)

**What We Did:**
Added final MVP audit documentation, refreshed README status, reconciled build journey project context with the implemented repository, and recorded the decision to mark the offline fixture MVP complete.

**Decisions & Rationales:**
The MVP is complete for offline deterministic fixtures, but production readiness still requires decimal/rounding policy, CI, richer validation, and integration decisions.

**Files Touched:**
`docs/audits/final-mvp-audit.md`, `README.md`, `BUILD_JOURNEY.md`

**Open Questions:**
Which post-MVP hardening item should be prioritized first?

**Next Steps:**
Begin post-MVP hardening.
