...
| CLI                                | Implemented as first-class offline interface | `src/cli`, package `bin`, README CLI section                                                      | `validate`, `run`, `batch`, `inspect`                                                                      | CLI tests cover help, input modes, output formats, strict mode, output file | Future engine capability must include CLI design or documented non-exposure. |
...
- Fixture/schema validation hardening for CLI `validate`.
...
| Fixture/schema validation         | Yes                                      | `validate` may add schema-only checks                                                   | Possible `--schema-only` later if justified                                             | Stronger schema documentation                                        | More precise validation errors                                                             | CLI tests for failures and exit codes                   |
