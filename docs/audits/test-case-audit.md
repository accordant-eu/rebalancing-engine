...
| T-12 | Info     | Future coverage | `min_trade_size` constraint enforcement (Slice 6) had no placeholder test                                                     | Original audit occurred before minimum trade suppression was implemented                               | Historical gap; now covered by trade and edge-case tests                                  | Add executable coverage when Slice 6 lands | **Fixed**    | `tests/trades.test.ts`, `tests/edge-cases.test.ts` |
